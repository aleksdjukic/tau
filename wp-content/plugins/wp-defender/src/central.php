...
class Central extends Component {
	...
	public function routing() {
		...
		if ( ! wp_verify_nonce( $nonce, $route ) ) {
			wp_send_json_error( [ 'message' => __( 'Invalid API request.', 'wpdef' ) ] );
		}
		...
		try {
			...
			if ( $is_private && ! $this->check_permission() ) {
				wp_send_json_error( [ 'message' => __( 'You shall not pass.', 'wpdef' ) ] );
			}
			if ( $is_private ) {
	...
	public function get_route( $method, $class ) {
		// Set 0 because user ID is lost when the user's session expires.
		$user_id = isset( $_REQUEST['interim-login'] ) ? 0 : get_current_user_id();
		$intention = hash( 'md5', sprintf( '%s.%s.%d', $class, $method, $user_id ) );
		if ( defined( 'DEFENDER_DEBUG' ) && constant( 'DEFENDER_DEBUG' ) === true ) {
			$intention = sprintf( '%s.%s.%d', $class, $method, $user_id );
		}
