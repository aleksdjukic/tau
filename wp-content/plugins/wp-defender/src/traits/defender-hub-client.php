...
trait Defender_Hub_Client {
	 ...
	 * @var string
	 */
	private static $global_ip_list_key = 'wpdef_global_ip_list';
	...
	public function fetch_global_ip_list() {
		$data = get_site_transient( self::$global_ip_list_key );

		if ( ! is_array( $data ) ) {
			$data = $this->make_wpmu_request( self::API_GLOBAL_IP_LIST );
		...
		if( ! is_array( $data ) ) {
			$this->log( 'Global IP API Error:', \WP_Defender\Controller\Firewall::FIREWALL_LOG );
			$this->log( var_export( $data, true ), \WP_Defender\Controller\Firewall::FIREWALL_LOG );
	...
	public function get_global_ip_list() {
		...
		if( $data === false ) {
			$data = $this->fetch_global_ip_list();
		}
	...
	public function set_global_ip_list( array $data ) {
		...
		$ret = set_site_transient( self::$global_ip_list_key, $value );
