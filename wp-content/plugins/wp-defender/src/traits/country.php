...
trait Country {
	...
	/**
	 * Fetch country from ip using external api service.
	 *
	 * @param string $ip IPV4 or IPV6 address.
	 *
	 * @return bool|array false on failure to fetch and array on success.
	 */
	public function ip_to_country_api( $ip ) {
		$url = 'https://api.redirect.li/v1/ip/' . $ip;
		...
		$options = [ 'timeout' => wp_doing_cron() ? 30 : 3 ];
		...
		if ( ! is_array( $body ) || ! isset( $body['countryName'] ) ) {
			return false;
		}

		return [
			'iso' => $body['countryCode'],
	...
	public function ip_to_country( $ip ) {
		...
		if ( ! $country ) {
			$country = $this->ip_to_country_api( $ip );
		}
