	...
	private function validate_trusted_proxies(): array {
		...
		if ( in_array( $this->http_ip_header, $custom_headers, true ) ) {
			...
			if ( ! in_array( $remote_addr, $trusted_proxies_ip, true ) ) {
				return [
					'error' => true,
					'message' => sprintf( __( 'At least one proxy ip should match with <strong>REMOTE_ADDR: %1s</strong>', 'wpdef' ), $remote_addr ),
				];
			}
			...
			foreach ( $trusted_proxies_ip as $ip ) {
				if ( ! filter_var( $ip, FILTER_VALIDATE_IP ) ) {
					return [
						'error' => true,
						'message' => sprintf( __( '%1s is not a valid IP address', 'wpdef' ), $ip ),
					];
				}
			}
		}
