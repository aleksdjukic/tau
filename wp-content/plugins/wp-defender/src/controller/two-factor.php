...
class Two_Factor extends Controller {
	...
	public function verify_otp_login_time() {
		...
		if ( ! wp_verify_nonce( $_POST['_wpnonce'], 'verify_otp' ) ) {
			// Todo: maybe display a message using wp_die()?
			return;
		}
		// Base params.
		$params = [
		...
		// Case with empty user data. Defender displays wp_die() here and for the 2 lower cases, so it doesn't make sense to display render_otp_screen() with spoofed user data.
		if ( empty( $user_id ) ){
			wp_die( __( 'Empty user data.', 'wpdef' ) );
		}
		...
		$params['token'] = $this->get_token( $user_id );
		// Get Auth method.
	...
	private function render_otp_screen( array $params = [] ) {
		...
		if ( 0 === count( $params['providers'] ) ) {
			// Since 3.5.0.
			$params['error'] = new WP_Error( 'opt_fail', 	__( 'No providers.', 'wpdef' ) );
		...
		return array_merge(
			[
				...
				'new_feature' => '<span class="sui-tag sui-tag-beta margin-right-10">' . __( 'Beta', 'wpdef' ) . '</span>'
					. sprintf(
					/* translators: %s: link */
						__( 'Web Authentication is now available. Enable users to authenticate login using their device\'s built-in fingerprint, facial recognition, or an external hardware security key. <a target="_blank" href="%s">Learn more</a>', 'wpdef' ),
