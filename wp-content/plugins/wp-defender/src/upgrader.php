...
class Upgrader {
	...
	public function run(): void {
		...
		if ( version_compare( $db_version, '3.6.0', '<' ) ) {
			$this->upgrade_3_6_0();
		}
...
Your temporary password is {{passcode}}. To finish logging in, copy and paste the temporary password into the Password field on the login screen.';
	 ...
	 * @return void
	 */
	private function upgrade_3_4_0(): void {
		$model = wd_di()->get( Blacklist_Lockout::class );
		$model->global_ip_list = false;
		$model->save();
	}
	...
	/**
	 * Upgrade to 3.6.0: Add the modal "What's new".
	 *
	 * @return void
	 */
	private function upgrade_3_6_0(): void {
		update_site_option( Feature_Modal::FEATURE_SLUG, true );
	}
}