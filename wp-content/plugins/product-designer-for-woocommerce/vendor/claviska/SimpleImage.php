	...
	public function textBox( $text, $options) {
		...
		if ($align <> 'justify') {
			foreach ($lines as $key => $line) {
				if ( $align == 'top' ) {
					$line = trim($line); // If is justify = 'center'
				}
	...
	*/
	private function textSeparateWords( $text) {
		...
		foreach ($text as $key => $line) {
			$newText = array_merge($newText, explode(' ', $line), array(PHP_EOL));
		}
	...
	public static function adjustColor( $color, $red, $green, $blue, $alpha) {
		...
		// Adjust each channel
		return self::normalizeColor(array(
		'red' => $color['red'] + $red,
	...
	public function extractColors( $count = 5, $backgroundColor = null) {
		...
		if ($backgroundColor) {
			...
			$backgroundColor = \League\ColorExtractor\Color::fromRgbToInt(array(
	...
	public static function normalizeColor( $color) {
		...
		if ($color === 'transparent') {
			$color = array('red' => 0, 'green' => 0, 'blue' => 0, 'alpha' => 0);
		}
